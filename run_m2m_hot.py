"""Evaluate HOT on the many-to-many Douban benchmark with the new M2M metrics."""
...
def main():
    ...
    hot = PlanetAlign.algorithms.HOT(alpha=0.5, lp=0.1).to("cpu")
    hot.train(dataset=dataset, gids=GIDS, use_attr=True,
              in_iters=5, out_iters=30, save_log=False, verbose=True)

    S = hot.S.detach().to(torch.float32).cpu()
    pred = similarity_to_pred_entities(S, gt_entities)
    m2m = many_to_many_scores(gt_entities, pred, metrics=M2M_METRICS)
    one_to_one = hot.test(dataset=dataset, gids=GIDS, metrics=ONE_TO_ONE_METRICS)

    print("\n" + "=" * 60)
    print("HOT on douban_m2m (1-1 reference metrics)")
    print("=" * 60)
    for k in ONE_TO_ONE_METRICS:
        print(f"  {k:<10}{one_to_one[k]:.4f}")

    print("\n" + "=" * 60)
    print("HOT on douban_m2m (new many-to-many metrics)")
    print("=" * 60)
    for k in M2M_METRICS:
        print(f"  {k:<10}{m2m[k]:.4f}")

    out_path = Path("logs") / "m2m_hot_results.json"
